  ...
  return (
    <Container className="md-container">
      <Head>
        ...
        <Row>
        <Title {...{'title':'Welcome to ', 'lang':'esen'}}/>  
          <TopBar {...{'title':'en', 'url':'/en', 'target':'/'}}/>
        </Row>
        <Row className={["justify-content-md-between", "product"].join(' ')}>
            <CardBlock {...{'data':Wizard['products'], 'lang':'en'}} ></CardBlock>
        </Row>
          {steps}
...
const buildSteps = (elements) => {
   const content = elements.map(section => (
    <Section key={section.id}{...{'section':section.component, 'lang':'en'}}/>
  ))
  return content
}